% Demo of linear MCMC reconstruction of ideal data for parallel beams.
...
OPL=load('FTOIdeal.mat');  %Load ideal optical depth data
OPL=OPL.OPL;
...
for i=1:NChain   % For each Markov chains
    ...
    for k=1:Nmax %For each sample
        ...
        if k>NDsc   %When Markov chains converges, only the area near the symmetry axis is filtered.
        xkbr=xk2D(1:15,:);
...
%           xk=max(0,xk);  %Non-negative constraint
            ...
            WM=QI-deta/2*QI*xk*(xk')*QI/(1+deta/2*(xk')*QI*xk);  %Smapling the covariance matrix            
            WMC=chol(WM);
...
XALLD=XCALL(:,NDsc+1:Nmax,:); %Discard samples which are not convergent 
% XALLD=max(0,XALLD);
